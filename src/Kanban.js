...
const Kanban = () => {
    ...
    const ChangeEditMode = (sectionId, taskId) => {
        for (let i = 0; i < data.length; i++) {
            if(data[i].id === sectionId) {
                for (let j = 0; j < data[i].tasks.length; j++) {
                    if (data[i].tasks[j].id === taskId) {
                        const reverseDataBoolean = !data[i].tasks[j].isEdit
                        
                        const section = data[i]
                        const indexOfTask = section.tasks.indexOf(section.tasks[j])
                        const task = {...section.tasks[j],isEdit:reverseDataBoolean}
                        section.tasks.splice(indexOfTask,1,task)
                        const obj = {...section,tasks:section.tasks}
                        const ArrayOfSection = [...data]
                        ArrayOfSection.splice(i,1,obj)
                                                        ...
                                                        >
                                                            <div style={{display: 'flex', justifyContent: 'space-between'}}>
                                                                <div>
                                                                    ...
                                                                    onBlur={(e)=>ChangeEditMode(section.id,task.id)}
                                                                ...
                                                                <svg onClick={()=>ChangeEditMode(section.id,task.id)}  width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
